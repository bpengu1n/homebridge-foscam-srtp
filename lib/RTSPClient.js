...
class RTSPClient extends EventEmitter {
  ...
  setup(uri, rtpPort, rtcpPort, ssrc, videoSrtp) {
    let self = this;
    return self.makeRequest({
      method: 'SETUP',
      uri: uri,
    ...
    }).then(result => { // Below is stream/response from device to send back/process
      let response = result.response;
      let plainData = result.data; // need to encrypt this AES128 HMAC SHA1-80
                              // HMAC = SHA1(concat(response+data))
      
      let hmacSig = crypto.createHmac('sha1', videoSrtp).update(Buffer.concat([response, data])).digest('hex');
      let data = crypto.createCipheriv('aes128', videoSrtp, ssrc).update(plainData, 'binary').final('binary'); // AES128 encrypted
