...
class FoscamStream extends EventEmitter {
    ...
    prepareStream(request, callback) {
        ...
        let video_srtp = Buffer.concat([request['video']['srtp_key'], request['video']['srtp_salt']]);
        let audio_srtp = Buffer.concat([request['audio']['srtp_key'], request['audio']['srtp_salt']]);
        ...
        self.transcoder.start().then(() => {
            return self.rtspClient.setup(self.rtspClient.video.uri, request['video']['proxy_rtp'], request['video']['proxy_rtcp'], ssrc, video_srtp).then(function(video) {
